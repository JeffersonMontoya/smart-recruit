...
export const RegisterPage = () => {
  ...
  const { loading, handleRegister } = useAuthActions();

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();

    try {
      const data = await handleRegister(nombre, email, password);
      toast.success(`¡Cuenta creada! Bienvenido, ${data.nombre}`);
    } catch (error: any) {
      toast.error(
    ...
    <div className="min-h-screen bg-zinc-950 flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-zinc-900 border border-zinc-800 rounded-xl p-8 shadow-2xl">
        <div className="text-center mb-8">
          ...
          <p className="text-zinc-400">Únete a Smart Recruit hoy mismo</p>
        </div>
